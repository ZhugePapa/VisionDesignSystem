...
- API migration strategy is old/Figma compatible first: keep old prop/event names as Vision public API unless there is a concrete conflict.
...
| loading | Direct/self-rendered | P1 | Done | Keeps old `size/color/text/label` API and spinner visuals. |
...
| alert | Self-rendered from correct source | P1 | Done | Migrated from `LeoAlert`; aligned to Figma type/description variants, subtle semantic backgrounds, 52/78px heights, and old action/closeable API. |
...
| date-picker | Self-rendered from correct source | P1 | Done | Migrated from `LeoDatePicker`; keeps date/range/open/shortcut API and removes the obsolete `readView` variant/prop per Figma. |
...
| message | Element Plus service adapter | P1 | Pending | Wrap `ElMessage`; expose old/Figma compatible Vision API. |
| notification | Element Plus service adapter | P1 | Pending | Wrap `ElNotification`; preserve old placements and icon/color APIs. |
...
5. Migrate remaining direct/special components: `markdown`, `rich-text-editor`, `input-search-box`, `toggle-button`.
