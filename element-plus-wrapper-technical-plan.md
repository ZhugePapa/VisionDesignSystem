...
1. 对外 API 先匹配 `leoht-design` / Figma 旧 API，而不是 Element Plus。
2. 旧 API 名默认就是 Vision Public API；语义化新名只作为必要别名或内部 adapter 概念。
3. Element Plus 的常用能力可以透传，但要有边界。
4. 透传属性使用 `inheritAttrs` 和 `v-bind="$attrs"`，但不要让关键视觉属性失控。
5. 事件名优先保持旧组件和 Vue 模板习惯，如 `v-model` / `update:modelValue` / kebab-case emit；TS 类型可额外提供 camelCase 别名。
6. 复杂组件可以暴露 Element Plus 原生类型，但最好用旧项目兼容类型或 Vision 类型再包装一层。
...
不要一开始就把 Element Plus 的所有 props 原样复制到 Vision API，否则以后会变成兼容负担。也不要为了追求更“语义化”的新命名而主动重命名旧 API；迁移期的稳定性优先级高于命名理想化。
...
| Element Plus 常用 API | 行为能力有价值，但不是 Vision 主语义 | 显式挑选后透出；若旧项目已有等价 API，优先旧名 | `disabled`、`htmlType`、`autofocus` |
| Element Plus 高级 API | 使用频率低、可能污染设计约束 | 放入 `elProps` 或 `$attrs` 透传，不写进主文档第一层 | `tag`、部分 popper 配置、内部行为参数 |

#### 1. 公开 API 以旧项目/Figma 兼容为主

组件文档、类型和示例都优先使用旧项目和 Figma 已存在的 API 名。所谓 Vision Public API 在迁移期不是重新发明一套名字，而是把旧 API 放进 Vision 组件边界内长期维护。
...
如果旧项目已经使用 `danger`、`state`、`htmlType`、`colorType`、`readView`、`closeable` 这类 API，默认保持这些名称，然后由 adapter 转换成 Element Plus 能理解的 props 和 class：
...
| 原生按钮类型 | `htmlType` | `native-type` | 旧/Figma API 保持，adapter 映射 |
...
| DatePicker | ElDatePicker | 当前采用旧项目自绘迁移；保留 date/range/open/shortcut API，移除 Figma 已取消的 `readView` 变体和 prop |
...
- 默认采用“兼容增强模式”：Vision 对外优先提供 `leoht-design` / Figma 兼容 API，Element Plus 作为内部行为底座。
- 旧 API 名默认是 Vision Stable API；不要把 `color/colorType` 自动改成 `tone`，不要把 `state` 自动改成 `status/visualState`，不要把 `htmlType` 自动改成 `nativeType`。
...
```text
Vision public API (old/Figma compatible)
  -> component adapter
...
| 层级 | 定义 | 示例 | 规则 |
| --- | --- | --- | --- |
| Vision Stable API | 业务优先使用的正式 API，迁移期默认沿用旧/Figma API | `variant`、`size`、`danger`、`state`、`colorType`、`readView` | 文档主入口，长期稳定 |
| Vision Enhancement API | Element Plus 没有但 Vision 需要的能力 | `iconOnly`、`density`、自定义 loading 图形 | 可进入主文档或进阶文档 |
| Selected Element Plus API | 行为稳定且不会破坏视觉约束的底层能力 | `disabled`、`clearable`、`autofocus` | 显式暴露，写清映射关系；若旧项目已有等价 API，优先旧名 |
| Escape Hatch | 高级场景临时透传 | `elProps`、`popperProps`、`dialogProps` | 放在 API 表末尾，不作为推荐用法 |

AI 生成组件时必须先读取旧组件 props/events/slots 和 Figma API，再确定 Vision Stable API；只有旧项目没有对应能力时，才新增语义化 API 或 Element Plus 选定 API。
...
| 设计语义 | 先沿用旧/Figma 命名，再在内部映射 | `danger` 映射危险语义，`colorType` 映射颜色 token，`state` 映射视觉/校验状态 |
...
export function resolveElementButtonProps(props: VisButtonProps) {
  ...
  return {
    ...
    disabled: props.disabled || props.loading,
    nativeType: props.htmlType ?? elProps.nativeType,
  }
}
```
...
| 状态 | `disabled/loading/state` | 对应底层 props | 行为复用，视觉由 Vision 控制 | stable |
| 原生行为 | `htmlType` 等 | 对应底层 props | 旧/Figma API 保持，adapter 映射 | stable |
...
node skills/leoht-component-migrator/scripts/audit-leoht-migration.mjs \
  --old /Users/leiwang/Documents/Project/LeohtDesign/leoht-design \
  --new .
```
